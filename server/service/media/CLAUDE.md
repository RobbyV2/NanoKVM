...
- The UVC MJPEG payload spec requires YCbCr; the black frame is a cached three-component JPEG, never grayscale. A camera worker encodes the frame for every declared geometry before it first polls the node (`warmFallback`), so a STREAMON finds its frame in the cache; the pure Go encoder is slow on this core and must never sit between the host's STREAMON and the first frame.
