...
| runtime | `/var/run/exit0-hev.pid`, `/var/run/exit0-wstunnel.pid`, `/var/run/exit0.lock`, `/tmp/exit0-hev.log`, `/tmp/exit0-wstunnel.log` |
...
`10.7.1`. The Windows command is `irm -Headers @{Authorization='Bearer $TOKEN'} <scheme>://$KVM/exit/0/client.ps1 | iex`
...
Then §3 step 3.3 through this mode as well. Pass: all three platforms verify the hash, run 10.7.1, the
reverse listener appears, UI connected, the consumer reaches the internet (§3) in Mode B.
