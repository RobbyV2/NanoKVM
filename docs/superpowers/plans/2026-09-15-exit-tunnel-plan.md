...
**Done (W5).** The four branches merged into `main` without conflicts. Wired in `wire.go` (counter to mux and front door only; `connecting` from `Mux.Connecting()`); the embed names exactly the four clients; every token-gate 404 goes through one helper with a real-listener identity test; the mode selector works while enabled (the manager's live switch is safe, the connected exit drops); `dns.redirected` is carried across converges in `/var/run/exit<n>.dns_redirected` since the chain flush zeroed it every 30 s; `conformance_test.go` runs both clients through every listed case and both pass under `-race`, the only defects it found being in its own harness. HELLO strings are confirmed `u8 len + utf8` on all five implementations and written into the spec, with every other deviation the Wave 1 reports named. `make app` and `make exit` pass; the web checks pass.
