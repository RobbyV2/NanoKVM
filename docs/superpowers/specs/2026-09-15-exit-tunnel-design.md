...
GET    /:slot/commands                 templated commands for both modes and three platforms, plus fingerprint,
                                       wstunnelVersion, wstunnelRepo and wstunnelLatest (two entries: windows, linux)
...
| `server/service/exit/commands.go` | new: command templating, pinned wstunnel table with SHA-256 per asset, latest-release command for windows and linux, fingerprint |
