...
Integration, on the host: `go test -race -tags conformance -run Conformance ./service/exit/` runs the Python and Perl clients as subprocesses against the real mux and front door (echo, 20 MB both ways with sha256, half close, OPEN_FAIL 0x05, UDP ASSOCIATE echo, supersede); it needs a non-loopback IPv4 address on the host because the policy denies 127/8 everywhere. PowerShell is exercised on Windows from `clients/testdata/windows-checklist.md`. The Mode B proxy against a real `wstunnel` binary and end-to-end `curl --socks5` through hev remain live-test items.
