...
| `server/service/exit/downstream.go` | new: runs `S94exit` and `S30rndis` through the Runner, parses `status`, prefers `/etc/init.d` and falls back to the shipped copy under `/kvmapp/system/init.d` |
