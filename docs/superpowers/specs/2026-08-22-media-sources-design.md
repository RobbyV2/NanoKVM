...
Until 2 and 3 return, the two-camera language in this document is a hypothesis, and the slot maximum in the profile schema stays at whatever the transcript supports.

## Risks

**R4.1 Two UVC functions cannot coexist on this silicon.** The whole multi-source premise rests on a composite gadget carrying two UVC interfaces, and nothing in this tree has instantiated even one. If `f_uvc` allocates a control status interrupt IN, two cameras plus HID needs seven IN endpoints against six FIFOs and cannot bind at all. Retires when a transcript from a device flashed with the `f74b732` image shows two `uvc.*` functions linked into `configs/c.1` alongside `hid.GS0` through `hid.GS2`, a successful UDC bind, `lsusb -v` from the attached host listing two camera functions with their endpoint counts, and both cameras opened simultaneously by two applications on that host.
