const fr = {
  translation: {
    ...
    devices: {
      ...
      revoked: {
        released: 'Le partage a été arrêté',
        lease_expired: 'Le bail a expiré avant le retour de ce navigateur',
        admin_disconnect: 'Un administrateur a déconnecté toutes les sources',
        slot_removed: 'L’emplacement a été supprimé',
        slot_changed: 'L’emplacement a été reconfiguré',
        taken_over: 'Un administrateur a repris cet emplacement'
      },
      usb: {
    ...
    settings: {
      ...
      display: {
        ...
        powerCycleUnverified:
          'L’écriture n’a pas été vérifiée : la puce vidéo garde ce qu’elle contient maintenant tant que cet appareil n’est pas physiquement débranché de l’alimentation puis rebranché.',
        ...
        mismatchTitle: 'Écrit et relu',
        mismatchWritten: 'Écrit',
        mismatchRead: 'Relu',
      ...
      presentation: {
        ...
        hostUnbound: 'Contrôleur non lié',
        hdmiState: 'Entrée HDMI',
        hdmiSignal: 'Signal présent',
        hdmiUnreported: 'Aucun rapport de capture pour l’instant',
        endpoints: 'Endpoints',
        fifos: 'Emplacements FIFO',
        pending: 'Modifications en attente',
        pendingEdits: 'Modifications d’identité non enregistrées',
        pendingProfile: '{{profile}} est sélectionné mais pas appliqué',
        pendingNone: 'Aucune',
        lastApply: 'Dernière application',
        applyFailed: 'Échec sur {{profile}} le {{time}}',
        applyClean: 'Aucun échec enregistré',
        lastKnownGood: 'Dernier état bon connu',
        rollbackTarget: 'Cible du retour arrière',
        rollbackNone: 'Aucune',
        powerCyclePending:
          'Le contrôleur a été retiré à l’hôte. Éteignez puis rallumez l’ordinateur connecté pour récupérer le périphérique.',
        ...
        applyNoHid:
          'Aucune fonction HID ne subsiste après cette application. Le clavier et la souris cesseront de fonctionner.',
        applyRollback: 'Une application ratée revient à {{profile}}.',
        recoveryPowerCycle:
          'Aucun HID ne survit à cette application : un hôte qui cesse de répondre ne pourra être récupéré qu’en coupant puis rétablissant son alimentation.',
        recoveryReboot:
          'Une interface disparaît du périphérique composite ; l’hôte peut avoir besoin d’un redémarrage pour relier le reste.',
        recoveryHdmiReset:
      ...
      passthrough: {
        ...
        inactiveDesc:
          'Aucune session en cours. Le clavier, la souris et les médias virtuels fonctionnent normalement.',
        device: 'Périphérique',
        busId: 'ID de bus',
        speed: 'Vitesse',
        exporter: 'Exportateur',
        local: 'Importé en tant que',
        ...
        busIdLabel: 'ID de bus sur votre machine',
        busIdHint:
          'Le busid que usbip list -l affiche pour le périphérique, par exemple {{example}}.',
        start: 'Démarrer le passthrough',
        stop: 'Arrêter le passthrough',
        startTitle: 'Démarrer le passthrough USB ?',
        startDevice: 'Le NanoKVM va importer {{busId}} depuis {{exporter}}.',
        startHid:
        ...
        steps: {
          modprobe: {
            title: 'Charger le pilote côté exportateur',
            desc: 'usbip-host permet à votre noyau de céder un périphérique local. Il n’est pas chargé par défaut.'
          },
          list: {
            title: 'Trouver le périphérique',
            desc: 'Affiche chaque périphérique local avec son busid et son couple fabricant:produit. Notez le busid de celui qui vous intéresse.'
          },
          bind: {
            title: 'L’attacher à usbip',
            desc: 'Retire le périphérique de son pilote habituel : il cesse de fonctionner sur cette machine jusqu’au détachement.'
          },
          serve: {
            title: 'Le publier',
            desc: 'usbipd reste au premier plan et attend que le NanoKVM importe le périphérique.',
            notice:
              'Le usbipd standard n’a pas d’option d’adresse d’écoute et écoute sur toutes les interfaces. Gardez le port {{port}} fermé sur votre pare-feu et laissez seulement le tunnel ci-dessous l’atteindre.'
          },
          tunnel: {
            title: 'Le pointer vers le NanoKVM',
            desc: 'Un tunnel SSH inverse : le port {{port}} sur la boucle locale du NanoKVM devient le usbipd de cette machine. Laissez-le tourner pendant toute la session.'
          },
          exporter: {
            title: 'Utiliser ceci comme exportateur',
            desc: 'Mettez ceci dans le champ exportateur ci-dessus, saisissez l’ID de bus, puis démarrez la session.'
          },
          unbind: {
            title: 'Rendre le périphérique',
            desc: 'Une fois la session arrêtée, ceci rend le périphérique à son pilote habituel sur cette machine.'
          }
        }
      },
      mcp: {
      ...
      network: {
        ...
        bridge: {
          ...
          inboundWeak:
            "La vérification entrante n'a réussi que parce que NanoKVM s'est connecté à lui-même. Cela prouve que le service web écoute et qu'il est joignable localement, pas qu'une requête venue du réseau lui parvient.",
          noCarrier:
            'Aucun lien sur {{port}}. Le pont n’a aucun chemin vers le réseau tant qu’un câble n’est pas branché.',
          loop: 'Le routeur est également appris sur {{port}} : ce port est donc un second chemin vers le même réseau. Le spanning tree est désactivé, rien ici ne cassera la boucle : débranchez l’un des deux chemins.',
          failedNotice:
      ...
      exit: {
        ...
        commands: {
          ...
          latestDesc:
            "Résout la dernière version de wstunnel sur GitHub à l'exécution de la commande et vérifie le téléchargement avec le checksums.txt de cette version ; ce n'est donc pas la version épinglée {{version}} et aucun hachage épinglé n'est inclus.",
          latestFallback:
