const ru = {
  translation: {
    ...
    settings: {
      ...
      exit: {
        title: 'Выходной туннель',
        description:
          'Превращает сетевой интерфейс USB в выход в интернет для целевого компьютера: его трафик уходит через ваше устройство, которое подключается по этому веб-адресу.',
        loading: 'Загрузка...',
        noSlots: 'На этом устройстве не настроен ни один выходной слот.',
        slot: 'Слот {{slot}}',
        enable: 'Выходной туннель',
        pending: 'применяется...',
        statusStale: 'состояние не обновляется',
        enableConfirm: 'Включить выходной туннель?',
        disableConfirm: 'Выключить выходной туннель?',
        reenumerate:
          'USB-гаджет переподключается, чтобы целевой компьютер получил новый шлюз и DNS. Клавиатура, мышь, камера и виртуальный диск пропадают примерно на две секунды.',
        okBtn: 'Да',
        cancelBtn: 'Нет',
        copy: 'Копировать',
        copied: 'Скопировано',
        ...
        status: {
          title: 'Состояние',
          disconnect: 'Отключить выход',
          disconnectDesc:
            'Разрывает соединение с подключённым выходным устройством. Его клиент переподключится сам, если вы не остановите его заранее.',
          tunnel: 'Туннель',
          uptime: 'работает {{uptime}}',
          lastConnected: 'последнее подключение {{time}}',
          never: 'никогда',
          peer: 'Выходное устройство',
          noPeer: 'Выходное устройство не подключено',
          peerChanged: 'Сменило {{addr}} в {{time}}',
          nic: 'Сетевой адаптер цели',
          nicUp: 'активен',
          nicDown: 'неактивен',
          nicMissing: 'Пока не определён',
          nicNone:
            'Текущий USB-профиль не содержит сетевой функции. Включите сетевой адаптер по USB в разделе «Сеть», чтобы у целевого компьютера появился сетевой адаптер.',
          upstream: 'Интернет',
          reachable: 'Доступен',
          unreachable: 'Недоступен',
          latency: '{{ms}} мс',
          notProbed: 'Не проверяется, пока не подключён выход',
          downstream: 'Нисходящий путь',
          downstreamOff: 'Выключен, пока туннель отключён',
          downstreamDegraded:
            'Часть нисходящего пути отсутствует. Сторожевой таймер повторяет попытку каждые 30 секунд; журналы ниже показывают, что не удалось.',
          downstreamTip: {
            forward: 'Пересылка IPv4 включена',
          ...
          dns: 'DNS',
          dnsStats: '{{queries}} запросов, {{failures}} неудачных, {{redirected}} перенаправлено',
          traffic: 'Трафик',
          trafficUp: '{{bytes}} к выходу',
          trafficDown: '{{bytes}} от выхода'
        },
        token: {
          title: 'Токен',
          history:
            'Входит в каждую команду ниже, поэтому попадает в историю оболочки выходного устройства.',
          regenerate: 'Перевыпустить',
          regenerateConfirm: 'Перевыпустить токен?',
          regenerateDesc:
            'Сначала остановите работающий клиент. Все подключённые выходы будут отключены, а старые команды перестанут работать.'
        },
        mode: {
          title: 'Режим',
          native: 'Родной',
          wstunnel: 'wstunnel',
          nativeDesc:
            'На выходном устройстве работает небольшой скрипт, загруженный с этого NanoKVM. Устанавливать ничего не нужно.',
          wstunnelDesc:
            'Выходное устройство загружает закреплённый бинарный файл {{version}} и проверяет его SHA-256 перед запуском.',
          whileEnabled:
        ...
        commands: {
          ...
          warnDownload:
            'Команда загружает скрипт с этого NanoKVM и запускает его от имени вашего пользователя.',
          warnSecret:
            'Она содержит секрет, управляющий интернетом целевого компьютера, и сохранится в истории вашей оболочки.',
          warnReach:
            'Всё, что доступно выходному устройству, становится доступным с целевого компьютера.',
          warnFingerprint:
            'Выход подключается только после проверки отпечатка сертификата {{fingerprint}}.',
          warnCleartext:
            'Эта страница отдаётся по обычному http: токен и весь трафик между выходом и NanoKVM передаются в открытом виде.',
          warnWstunnelUnverified:
            'wstunnel не умеет закреплять отпечаток сертификата. Без сертификата, подписанного ЦС, его соединение с NanoKVM не аутентифицировано.',
          ...
          warnFingerprintReaddressed:
            'Скрипт закрепляет отпечаток сертификата {{fingerprint}} этого NanoKVM. Если {{host}} предъявит другой сертификат, например прокси, завершающий TLS, exit откажется подключаться; пропускайте TLS до NanoKVM или подключайтесь напрямую.',
          unavailable: 'Для этой платформы команды нет.',
          regenerateHint:
            'Выход подключился. Когда вставленная команда больше не нужна, перевыпустите токен; выход придётся перезапустить с новой командой.',
          security:
            'Целевой компьютер уже имеет доступ к веб-интерфейсу, SSH и VNC этого NanoKVM по адресу USB-сети. Эта функция этого не меняет.',
          latestTitle: 'Последний wstunnel',
          latestDesc:
            'При запуске команды определяет новейший выпуск wstunnel на GitHub и сверяет загрузку с checksums.txt этого выпуска, так что это не закреплённая версия {{version}} и закреплённого хеша в ней нет. Только Windows и Linux.',
          latestFallback: 'Если загрузка не удалась, скачайте его с github.com/erebe/wstunnel',
          latestUnavailable:
            'Для macOS команды получения последней версии нет. Используйте закреплённую команду выше или ссылку на репозиторий.'
        },
        advanced: {
          title: 'Дополнительно',
          dns: 'DNS-серверы',
          dnsTip:
            'До четырёх резолверов, доступных через выход. Целевой компьютер получает NanoKVM в качестве резолвера, и каждый открытый DNS-запрос перенаправляется сюда.',
          dnsAdd: 'Добавить сервер',
          dnsRemove: 'Удалить сервер',
          dnsInvalid: 'Вводите только IP-адреса',
          dnsDenied: 'Адреса loopback, link-local, multicast и 198.18/15 недостижимы через выход',
          dnsPrivate: 'Для приватного резолвера нужно включить Разрешить приватные назначения',
          mtu: 'MTU',
          mtuTip: 'От {{min}} до {{max}}. 1280 оставляет место для заголовков туннеля.',
          allowPrivate: 'Разрешить частные адреса назначения',
          allowPrivateTip:
            'Позволяет целевому компьютеру достигать собственной локальной сети выходного устройства: 10/8, 172.16/12, 192.168/16, 100.64/10 и fc00::/7. Выключено — они отклоняются.',
          pinPeer: 'Закрепить за первым выходом',
          pinPeerTip:
            'Отклоняет подключение с другого адреса, пока не будет выключено. Иначе новое действительное подключение заменяет старое.',
          save: 'Сохранить',
          saved: 'Сохранено'
        },
        logs: {
    ...
    error: {
      ...
      panel: 'Эта панель перестала работать',
      retry: 'Повторить',
